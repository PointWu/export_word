<template>
    <div>
                    ...
                    <template slot-scope="scope">
                        ...
                        <el-tooltip effect="dark" content="编辑" placement="top" :enterable="false">
                            <el-button type="primary" icon="el-icon-edit" size="mini"
                                @click="getChnageInfo(scope.row.id)">
                            </el-button>
                        </el-tooltip>
                        <!-- 删除按钮 -->
        ...
        <!-- 修改用户的对话框 -->
        <el-dialog title="修改用户" :visible.sync="changeDialogVisible" width="50%" @close="changeClose">
            <!-- 内容主体区域 -->
            <el-form :model="changeForm" :rules="ruleFormRules" ref="changeFormRef" label-width="100px">
                <!-- 用户名 -->
...
<script>
    export default {
        ...
        methods: {
            ...
            // 修改用户
            async changeUser() {
                this.$refs.changeFormRef.validate(vaild => {
                    // 如果校验不合法，则返回
                    if (!vaild) return
                    // 校验合法，发起修改用户的网络请求
                })
            ...
            async removeUserById(id) {
                // 弹框提示用户是否删除
                const confirmResult = await this.$confirm('此操作将永久删除该用户, 是否继续?', '删除用户提示', {
